(function($, window, document,undefined) {
  ...
  cjjTable.prototype = {
        start:function(){
            var _this = this;
            var titlelistBox="";
            var titlesmall=_this.options.operation==0?"":"<th style='width:160px;'><input type='checkbox' name='al-choose' class='cjjTable-checkBoth dl-choose'></th>";
            ...
            for (var i = 0; i <firstPageNumber; i++) {
                var bodyBigBox="";
                var bodyBox=_this.options.operation==0?"":"<td style='width:160px;'><input type='checkbox' name='checkList' class='cjjTable-checkList'></td>";
                for(var x=0;x<_this.options.body.length;x++){
                    ...
                    if(_this.options.body.length>_this.options.title.length&&_this.options.display[x]!=undefined){
                        display = _this.options.display[x]*1==1?"display":"none";
                    }
                    bodyBox+="<td data-html='"+json[i][type]+"' class='"+type+"' style='display:"+display+"'>"+json[i][type]+"</td>";
                    bodyBigBox = bodyBox;
                }
                histroy_DD +="<tr data-number='"+i+"' class='new_productBox object_join_a'>"+bodyBigBox+"</tr>";
            ...
            "<td  colspan='"+_this.options.title.length+"'>"+
                "<div style='float:right;margin-left:10px;' class='tfootRight'>"+
                    ...
                    "</div>"+
                    "<div style='float:right'>"+
                          "<select><option value='5'>5</option><option value='10'>10</option><option value='20'>20</option><option value='50'>50</option></select>"
                    "</div>"+
                "</div>"
            "<td>"+
            "<tr>";
            _this.$element.html("<table width='100%' class='CJJ-Table am-table am-table-compact am-text-nowrap tpl-table-black'><thead>"+titlelistBox+"</thead><tbody>"+histroy_DDBox+"</tbody><tfoot>"+buttonTfoot+"</tfoot></table>");
        ...
        page:function(maxpagenumberBox,json,e) {
            ...
            if(json.length>0){
                for (var i =(Pagenumber-1)*pageNUmber; i < lastPage; i++) {
                    var bodyBigBox="";
                    var bodyBox=e.options.operation==0?"":"<td style='width:160px'><input type='checkbox' name='checkList' class='cjjTable-checkList''></td>";
                    for(var x=0;x<e.options.body.length;x++){
                        ...
                        if(e.options.body.length>e.options.title.length&&e.options.display[x]!=undefined){
                            display = e.options.display[x]*1==1?"display":"none";
                        }
                        bodyBox+="<td data-html='"+json[i][type]+"' class='"+type+"'  style='display:"+display+"'>"+json[i][type]+"</td>";
                        bodyBigBox = bodyBox;
                    }
