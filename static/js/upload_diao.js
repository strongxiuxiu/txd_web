...
function imgsUpload(imgObj, obj,imgNum) {
    ...
    $(".uploaddiao_add").click(function () {
        lastInput = $('.' + fileClass).last(); //得到最后的input,如果是IE，会生成多个, 这个input是工具人性质
        $('.' + fileClass).each(function (k, v) {
            $(this).attr('id', '');
        })
        lastInput.attr('id', 'fileUp_diaoIpt'); //只给最后一个input正确的ID值和label联动，不用label联动  IE9行不通，提交会失败
        ...
        lastInput.bind("change", function () {
                 ...
                 for(let ii of filesArr){
                   ...
                   console.log(text_type)
                     if(text_type == "jpg" || text_type == "png" || text_type == "epg" || text_type == "svg" || text_type == "gif" || text_type == "jpe"){
                        upImgs = 
                        "<div class='show-img-diao_div'><span><img style='width:15px; height:15px;' src='/static/img/close.png' /></span><img  style='width: 65px; height: 65px;'  src='" + imgSrc + "'/></div>";
                     }else if(text_type == "pdf"){
                        upImgs = "<div class='show-img-diao_div'><span><img style='width:15px; height:15px;' src='/static/img/close.png' /></span><img  style='width: 65px; height: 65px;'  src='/static/img/pdf.png'/></div>";
                     }else{
                         alert("只能上传pdf文件或图片")
                     }
                 }
              
                $("#labelId_objectdiao").before(upImgs);
                //$(".type-file-div").append(lastInput.clone()); //如果需要和IE方式一样的方式添加多个 input 去上传 打开这个注释

            } else { //IE9 IE8兼容
                var upImgs = "<div class='show-img-diao_div'><span><img style='width:15px; height:15px;' src='/static/img/close.png' /></span><img style='width: 80px; height: 80px;' class='ie-imgshow' src='./img/null.png' /></div>";
                $("#labelId_objectdiao").before(upImgs);
                lastInput[0].select();
            ...
            if(imgObj.length>=imgNum){$("#labelId_objectdiao").hide()}
            //添加新图片后需要new imgPre  重置 预览函数
            obj = new imgPre(imgObj);
            gbOption = obj;
            createPreDiv();
            //删除图标绑定删除事件
            $(".show-img-diao_div").each(function (k, v) {
...
function deleteImg(imgObj, k, obj) {
    ...
    if(imgObj.length<imgMaxNum){
        $("#labelId_objectdiao").show()
    }
    //删除点击的自己
    $(".show-img-diao_div:eq(" + k + ")").remove();
    //删除后重新绑定事件
    $(".show-img-diao_div").each(function (k, v) {
        $(this).find('span').unbind('click');
        $(this).find('span').bind("click", function () {
...
function imgPre(imgObj, flag) {
    ...
    if (flag) {
        ...
        this.imgObj.forEach(function (v, k) {
            showImgs += "<div class='show-img-diao_div'><img style='width: 80px; height: 80px;' src='" + v + "'/></div>";
        })
